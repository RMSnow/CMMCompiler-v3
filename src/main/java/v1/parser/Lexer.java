...
public class Lexer {
        ...
        if(!include(FilterChar, c)) {
            ...
            if(state == State.Normal) {
                ...
                if(inIdentifierSetFirst(c)) {
                    ...
                    type = Token.Type.NewStatement;
                } else if(c == '\n') {

                } else if(c == '\0') {
                    type = Token.Type.EndSymbol;
                } else {
                ...
                if (c == '.') {
                    ...
                    type = hasDot ? Token.Type.Decimal : Token.Type.Integer;
                    hasDot = false;
                    state = State.Normal;
                    moveCursor = false;
                }
                ...
                if(SignParser.inCharSet(c)) {
                    ...
                    for(String signStr:list) {
                        createTokenForSign(Token.Type.Sign, signStr, signOriginPos);
    ...
    private void createTokenForSign(Token.Type type, String value, int pos) {
        Token token = new Token(type, value, pos, lines);
        tokenBuffer.addFirst(token);
        buf = null;
    }
