...
import v1.parser.node.NNode;
import v1.parser.node.Node;
import v1.parser.node.TNode;
...
class CodeCreator {
    ...
    private void handleVarDecl(NNode root, Context context) {
        ...
        for (int i = 0; i < varListNode.getChildren().size(); i++) {
            ...
            if (varNode instanceof TNode && ((TNode) varNode).getSymbol() == TerminalSymbol.Identifier) {
                ...
                if (arrayIndex != -1) {
                    context.chunk.push(CodeChunk.Command.NewArray, var, arrayIndex);
            ...
            if (i+1 < varListNode.getChildren().size()) {
                ...
                if (varNode instanceof NNode) {
                    ...
                    context.chunk.push(CodeChunk.Command.Mov, var, val);
    ...
    private void handleIFStmt(NNode root, Context context) {
        ...
        if (root.getChildren().size() > 3) {
            ...
            context.chunk.push(CodeChunk.Command.JmpUnless, nextHolder, var);
            Context childContext = context.link();
            NNode node = (NNode) root.getChildren().get(2);
            handleStmtBlock(node, childContext);
            context.chunk.push(CodeChunk.Command.Jmp, endHolder);
            ...
            context.chunk.push(CodeChunk.Command.JmpUnless, endHolder, var);
            Context childContext = context.link();
            NNode node = (NNode) root.getChildren().get(2);
            handleStmtBlock(node, childContext);
            context.positionPlaceholder.setPosition(endHolder, context.chunk.getCurrentPostion());
    ...
    private void handleWhileStmt(NNode root, Context context) {
        ...
        handleExpr(condition, var, context);
        childContext.chunk.push(CodeChunk.Command.JmpUnless, endHolder, var);
        ...
        context.chunk.push(CodeChunk.Command.Jmp, checkConditionLocation);
        context.positionPlaceholder.setPosition(endHolder, context.chunk.getCurrentPostion());

    }
    private void handleBreakStmt(NNode root, Context context) {
        context.chunk.push(CodeChunk.Command.Jmp, context.jumpStack.getCurrentBreakLocation());
    }
    private void handleContinueStmt(NNode root, Context context) {
        context.chunk.push(CodeChunk.Command.Jmp, context.jumpStack.getCurrentContinueLocation());
    }
    private void handleReadStmt(NNode root, Context context) {
        ...
        if (root.getChildren().size() > 2) {
            ...
            context.chunk.push(CodeChunk.Command.Get, var, id, i);
            context.chunk.push(CodeChunk.Command.Read, var);
            context.chunk.push(CodeChunk.Command.Set, id, var, i);
            context.variablePool.freeIndex(i);
            context.variablePool.freeIndex(var);
        } else {
            context.chunk.push(CodeChunk.Command.Read, id);
        }
    ...
    private void handleWriteStmt(NNode root, Context context) {
        ...
        context.chunk.push(CodeChunk.Command.Write, var);
        context.variablePool.freeIndex(var);
    }
    private void handleAssignStmt(NNode root, Context context) {
        ...
        switch (sign.getValue()) {
            case "=": {
                ...
                context.chunk.push(CodeChunk.Command.Mov, res, num);
            ...
            case "+=": {
                int num = context.variablePool.createIndex();
                handleExpr(expr, num, context);
                context.chunk.push(CodeChunk.Command.Add, res, num);
                context.variablePool.freeIndex(num);
                break;
            }
            case "-=": {
                int num = context.variablePool.createIndex();
                handleExpr(expr, num, context);
                context.chunk.push(CodeChunk.Command.Sub, res, num);
                context.variablePool.freeIndex(num);
                break;
            }
            case "*=": {
                int num = context.variablePool.createIndex();
                handleExpr(expr, num, context);
                context.chunk.push(CodeChunk.Command.Mul, res, num);
                context.variablePool.freeIndex(num);
                break;
            }
            case "/=": {
                int num = context.variablePool.createIndex();
                handleExpr(expr, num, context);
                context.chunk.push(CodeChunk.Command.Div, res, num);
                context.variablePool.freeIndex(num);
                break;
            }
            case "%=": {
                int num = context.variablePool.createIndex();
                handleExpr(expr, num, context);
                context.chunk.push(CodeChunk.Command.Mod, res, num);
                context.variablePool.freeIndex(num);
                break;
            }
            case "&&=": {
                int num = context.variablePool.createIndex();
                handleExpr(expr, num, context);
                context.chunk.push(CodeChunk.Command.And, res, num);
                context.variablePool.freeIndex(num);
                break;
            }
            case "||=": {
                int num = context.variablePool.createIndex();
                handleExpr(expr, num, context);
                context.chunk.push(CodeChunk.Command.Or, res, num);
                context.variablePool.freeIndex(num);
                break;
            }
            default:
        ...
        if (hasIndex) {
            ...
            context.chunk.push(CodeChunk.Command.Set, var, res, index);
            ...
            for (int i = 1; i < size; i++) {
                ...
                switch (sign.getValue()) {
                    ...
                    case "%":
                        ...
                        break;
                    case ">":
                        context.chunk.push(CodeChunk.Command.Gt, res, num);
                        break;
                    case ">=":
                        context.chunk.push(CodeChunk.Command.Gte, res, num);
                        break;
                    case "<":
                        context.chunk.push(CodeChunk.Command.Lt, res, num);
                        break;
                    case "<=":
                        context.chunk.push(CodeChunk.Command.Lte, res, num);
                        break;
                    case "&&":
                        context.chunk.push(CodeChunk.Command.And, res, num);
                        break;
                    case "||":
                        ...
                        break;
                    case "==":
                        context.chunk.push(CodeChunk.Command.Equal, res, num);
                        break;
                    case "!=":
                        context.chunk.push(CodeChunk.Command.NotEqual, res, num);
                        break;
                    default:
                        throw new IntermediateException("unknown operation '" + sign.getValue() + "'");
                }
        ...
        if (node instanceof TNode) {
            ...
            switch (sign.getValue()) {
                case "+":
                    ...
                    break;
                case "-":
                    context.chunk.push(CodeChunk.Command.Opposite, res, res);
                    break;
                case "!":
                    context.chunk.push(CodeChunk.Command.Not, res, res);
                    break;
                default:
                    throw new IntermediateException("unknown operation '" + sign.getValue() + "'");
            }
    ...
    private void handleValue(NNode root, int res, Context context) {
        ...
        if (node.getSymbol() == TerminalSymbol.Identifier) {
            ...
            if (root.getChildren().size() >= 2) {
                ...
                handleExpr(index, i, context);
                context.chunk.push(CodeChunk.Command.Get, res, num, i);
                context.variablePool.freeIndex(i);
            } else {
                int num = context.recorder.getVarIndex(node.getValue());
                context.chunk.push(CodeChunk.Command.Mov, res, num);
            ...
            CodeChunk.ImmediateNumber number = new CodeChunk.ImmediateNumber();
            switch (node.getSymbol()) {
                case Int:
            ...
            context.chunk.push(CodeChunk.Command.Mov, res, number);
        }

    }

    private CodeChunk.ImmediateNumber initialize(PrimaryType type) {
        CodeChunk.ImmediateNumber number = new CodeChunk.ImmediateNumber();
        switch (type) {
            case Int:
                number.type = CodeChunk.ImmediateType.Integer;
